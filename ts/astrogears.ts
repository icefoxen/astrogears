...
function find_ratios_recursive_silliness3(ratio: number, min_wheel_teeth: number, min_pinion_teeth: number, max_teeth: number) {
  for(let wheel1 = min_wheel_teeth; wheel1 < max_teeth + 1; wheel1++) {
    for(let pinion1 = min_pinion_teeth; pinion1 < max_teeth + 1; pinion1++) {
      ...
      find_ratios_recursive_silliness2(ratio1 * ratio, wheel1, pinion1, max_teeth);
    }
  }
}

function find_ratios_recursive_silliness2(ratio: number, min_wheel_teeth: number, min_pinion_teeth: number, max_teeth: number) {
  for(let wheel1 = min_wheel_teeth; wheel1 < max_teeth + 1; wheel1++) {
    for(let pinion1 = min_pinion_teeth; pinion1 < max_teeth + 1; pinion1++) {
      let ratio1 = (wheel1/pinion1);
